...
namespace motion_planner {
    ...
    PathPlanner::Options::Options()
            : algorithm_(default_algorithm), reward_temporal_coherence_(default_reward_temporal_coherence),
    ...
    PathPlanner::run(Eigen::Vector2f goal_in_map_frame,
        ...
        while ((tree.nodes_.size() + number_of_skipped_nodes) < options.number_of_random_samples_) {
            ...
            if (USE_RRT_STAR) {
                ...
                for (unsigned int i = 0; i < k; i++) {
                    ...
                    const float distance_from_node = L2_norm(node->position_(0), node->position_(1), random_row,
                                                             random_col);
            ...
            tree.nodes_.push_back(
                    Node(leaf_row, leaf_col, parent_node_index, accumulated_path_length_to_leaf)); // insert new leaf
            ...
            if (leaf_to_goal_distance <= options.goal_proximity_threshold_) {
                ...
                if (USE_INFORMED_RRT_STAR) {
                    ...
                    distribution_along_major_axis = UniformDistribution(0.0f,
                                                                        heuristic_sampling_domain_major_axis_length);
                    distribution_along_minor_axis = UniformDistribution(0.0f,
                                                                        heuristic_sampling_domain_minor_axis_length);

                    T_sampling_domain_to_grid_frame = Eigen::Affine3f::Identity();
                    T_sampling_domain_to_grid_frame.pretranslate(
                            Eigen::Vector3f(0, -(heuristic_sampling_domain_minor_axis_length) / 2.0, 0.0f));
        ...
        MapPath map_path; // path in the "map" coordinate frame
        ...
        nodes_on_path.push_front(Node(goal_row, goal_col, leaf_index_of_best_path_to_goal,
                                      accumulated_path_length_to_goal)); // insert the goal as the last node in the path
        ...
        while (true) {
            ...
            map_path.push_front(Eigen::Vector2f(node_in_map_frame(0), node_in_map_frame(1)));
        ...
        return {true, tree, map_path, grid_path};
    }
...
#define SHOW_PATH_TO_GOAL
        ...
        if (show_path_to_goal) {
            ...
            for (unsigned int j = 1; j < path.size(); j++) {
                ...
                geometry_msgs::Point p0;
                p0.x = v0_in_map_frame(0);
                p0.y = v0_in_map_frame(1);
                p0.z = 0.0;
                marker.points.push_back(p0);

                geometry_msgs::Point p1;
                p1.x = v1_in_map_frame(0);
                p1.y = v1_in_map_frame(1);
                p1.z = 0.0;
                marker.points.push_back(p1);
            }
            marker_messages.push_back(marker);
